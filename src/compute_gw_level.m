...
function [gw_ts_out] = compute_gw_level(...
    ...
    if numel(varargin)>0; maxD = varargin{1}; else; maxD = 3000; end
    ...
    [r,c] = size(well_depth);
    if r > c; well_depth = well_depth'; [~,c] = size(well_depth); end
    if c ~= size(gw_in,2)
    ...
    if sum(wellsLTmaxD) > 0
        temp_wd = well_depth(:,wellsLTmaxD); 
        temp_gw_in = gw_in(:,wellsLTmaxD);
        
        temp_wd = repmat(temp_wd,numOfObs,1);
        gw_out(:,wellsLTmaxD) = (maxD-temp_wd)+temp_gw_in;
    end
